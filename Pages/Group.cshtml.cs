...
namespace GharchNews.Pages
{
    public class GroupModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(string group, int pageId = 1)
        {
            ...
            int itemCount = _context.Reports.Include(r => r.ReportGroup).Where(r => r.ReportGroup.GroupName == group && r.IsSuccess == true).Count();
            ...
            {
                Reports=await _context.Reports.Include(r=>r.ReportGroup).Where(r=>r.IsHotNews==true && r.IsSuccess == true).OrderByDescending(r=>r.HotNewsDate)
                    .Skip(skip).Take(take).ToListAsync();
                ViewData["GroupName"] = group;
                return Page();
            }

            Reports = await _context.Reports.Include(r => r.ReportGroup).Where(r => r.ReportGroup.GroupName == group && r.IsSuccess == true)
