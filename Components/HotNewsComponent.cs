...
namespace GharchNews.Components
{
    public class HotNewsComponent : ViewComponent
    {
        ...
        {
            Reports = await _context.Reports.Include(r=>r.ReportGroup).Where(r => r.IsHotNews == true && r.IsSuccess == true).OrderByDescending(r=>r.HotNewsDate).Skip(skip).Take(take).ToListAsync();
