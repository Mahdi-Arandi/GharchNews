...
namespace GharchNews.Components
{
    public class SpecialReportsComponent : ViewComponent
    {
        ...
        {
            Reports = await _context.Reports.Where(r => r.ReportGroup.GroupName != groupName && r.IsSuccess == true).OrderByDescending(r => r.CreateDate).Skip(skip).Take(take).ToListAsync();
